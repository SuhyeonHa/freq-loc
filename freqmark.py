...
class Params:
    ...
    def __init__(self):
        ...
        transforms.Resize((self.dino_image_size, self.dino_image_size)),
        transforms.ToTensor(),
        ])
...
class FreqMark:
    def __init__(self, args):
        self.message_bits = args.message_bits
        ...
        self.image_encoder = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14').to(self.device)
        ...
        self.mu = self.margin      # Hinge loss margin
        
        # Noise parameters for robustness
        self.eps1_std = 0.25  # Latent noise
        self.eps2_std = 0.06  # Pixel noise
    
    def _init_direction_vectors(self) -> torch.Tensor:
        """Initialize direction vectors as described in paper"""
        vectors = torch.zeros(self.message_bits, self.feature_dim)
        for i in range(self.message_bits):
            vectors[i, i] = 1.0  # One-hot encoding
        return vectors.to(self.device)
    
    def embed_watermark(self, image: torch.Tensor, message: torch.Tensor, img_size: int) -> torch.Tensor:
        """
        ...
        optimizer = optim.Adam([delta_m], lr=args.lr)
        ...
        for step in range(args.steps):
            ...
            eps1 = torch.randn_like(perturbed_latent) * self.eps1_std
            eps2 = torch.randn_like(watermarked_image) * self.eps2_std
            ...
            watermarked_image_2 = F.interpolate(watermarked_image_2, size=(img_size, img_size), mode="bilinear", align_corners=False)
            ...
            total_loss = (loss_m1 + loss_m2 + loss_m3 + 
                         args.lambda_p * loss_psnr + 
                         args.lambda_i * loss_lpips)
    ...
    def decode_watermark(self, watermarked_image: torch.Tensor) -> torch.Tensor:
        ...
        """
        watermarked_image = watermarked_image.to(self.device)
        ...
        with torch.no_grad():
            ...
            # Extract message using sign function (Equation 4)
            decoded_message = torch.sign(dot_products)
...
def run_freqmark_demo(args=None):
    ...
    # test_images = load_images_from_path('/mnt/nas5/suhyeon/datasets/DIV2K_train_HR', num_test_images, transform=transform)
    test_images = load_image(args.image_path, transform=args.transform)
    test_messages = generate_random_messages(args.num_test_images, args.message_bits)
    
    # Test with a small batch first
    batch_size = 1
    test_batch = test_images[0:batch_size].to(args.device)
    msg_batch = test_messages[0:batch_size].to(args.device)

    print(f"\nTesting with batch size: {batch_size}")
    
    # Embed watermarks
    print("Embedding watermarks...")
    watermarked_batch = freqmark.embed_watermark(test_batch, msg_batch, args.dino_image_size)
    ...
    # Calculate metrics
    bit_accuracy = freqmark.compute_bit_accuracy(msg_batch, decoded_batch)
    avg_psnr = np.mean([freqmark._compute_psnr(watermarked_batch[i:i+1], test_batch[i:i+1]) 
    ...
    fig, axes = plt.subplots(3, batch_size, figsize=(16, 9), squeeze=False)
    ...
    plt.tight_layout()
    plt.savefig("freqmark_outputs.png", dpi=300, bbox_inches='tight')
    ...
    return {
        ...
        **attacks_results,
        'watermarked_images': watermarked_batch,
...
if __name__ == "__main__":
    ...
    # Main demo
    demo_results = run_freqmark_demo(args=args)
    
    # Compare with paper
    compare_with_paper_results(demo_results)
    
    # Analyze frequency domain advantage
    freq_analysis = analyze_frequency_domain_advantage(args)
    
    print(f"\nKey findings:")
    print(f"- Achieved PSNR: {demo_results['psnr']:.2f} dB (Paper: 31.27 dB)")
